...
process.on('SIGTERM',()=>pool.end())+(params.length+1)+' OFFSET 
})
...
console.log('Database schema applied')
app.listen(Number(process.env.PORT||3000),()=>console.log('ForeignID API on :3000'))
process.on('SIGTERM',()=>pool.end())+(params.length+2)
  const r=await query(sql,[...params,pageSize,offset])
  res.json({data:r.rows,total:Number(count.rows[0]?.total||0),page,pageSize})+(params.length+1)+' OFFSET 
  const r=await query(sql,[...params,pageSize,offset])
  res.json({data:r.rows,total:Number(count.rows[0]?.total||0),page,pageSize})
})
...
process.on('SIGTERM',()=>pool.end())+(params.length+2)
  const r=await query(sql,[...params,pageSize,offset])
  res.json({data:r.rows,total:Number(count.rows[0]?.total||0),page,pageSize})+(params.length+2)+'
  const r=await query(sql,[...params,pageSize,offset])
  res.json({data:r.rows,total:Number(count.rows[0]?.total||0),page,pageSize})
})
