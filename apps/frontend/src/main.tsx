    ...
    return <>
      ...
      {detailTab==='Документы'&&<div className="grid2">
        <div className="panel pad"><h2>{editingDocId?'Редактировать документ':'Добавить документ'}</h2>
          ...
          {docs.map((d:any)=><div className="record" key={d.id}><span>{d.document_type} №{d.document_number} · до {d.expiry_date||'—'}</span><span><button type="button" onClick={()=>editDoc(d)}>Редактировать</button> {canDelete&&<button type="button" onClick={()=>deleteDoc(d.id)}>Удалить</button>}</span></div>)}
        </div>
      ...
      {detailTab==='Регистрация'&&<div className="panel pad"><h2>{editingRegistrationId?'Редактировать регистрацию':'Добавить регистрацию'}</h2>
        ...
        {regs.map((r:any)=><div className="record" key={r.id}><span>{r.registration_type} {r.registration_number?'№'+r.registration_number:''} · с {r.start_date||'—'} до {r.end_date||'—'}</span><span><button onClick={()=>editRegistration(r)}>Редактировать</button> {canDelete&&<button onClick={()=>deleteRegistration(r.id)}>Удалить</button>}</span></div>)}
      </div>}
