...
function App(){
  ...
  function Detail(){
    ...
    return <><button className="back" onClick={()=>setSelected(null)}>← К списку</button><div className="page-title-row"><div className="person-title">{f.photo_url?<img className="avatar" src={f.photo_url}/>:<div className="avatar placeholder">Фото</div>}<div><h1>{f.last_name} {f.first_name} {f.middle_name||''}</h1><p className="muted">{f.citizenship} · <span className="ok">Активен</span></p></div></div><div>{canWrite&&<button className="primary" onClick={startEdit}>Редактировать</button>} {canDelete&&<button onClick={()=>archive(f.id)}>Архивировать</button>}</div></div>{error&&<div className="error">{error}</div>}
      <div className="tabs">{['Основная информация','Документы','Виза / Разрешение','Регистрация','История'].map(x=><button key={x} className={detailTab===x?'active':''} onClick={()=>setDetailTab(x)}>{x}</button>)}</div>
      {detailTab==='Основная информация'&&<div className="grid3">
      ...
      {detailTab==='Документы'&&<div className="panel pad"><h2>Документы и файлы</h2>{selected.documents.map((d:any)=><div className="record" key={d.id}><span>{d.document_type} №{d.document_number} · до {d.expiry_date||'—'}</span><span><button type="button" onClick={()=>editDoc(d)}>Редактировать</button> <button type="button" onClick={()=>deleteDoc(d.id)}>Удалить</button></span></div>)}{selected.files?.map((x:any)=><div className="record" key={x.id}><span>{x.file_name}</span><span><a href={x.file_url} target="_blank" rel="noreferrer">Открыть</a> <button type="button" onClick={()=>deleteFile(x.id)}>Удалить</button></span></div>)}</div>}
      {detailTab==='Виза / Разрешение'&&<div className="panel pad"><h2>Визы и разрешения</h2>{selected.visas.length?selected.visas.map((v:any)=><div className="record" key={v.id}><span>{v.visa_type} {v.visa_number?'№'+v.visa_number:''} · до {v.end_date}</span><span><button onClick={()=>{setEditingVisaId(v.id);setVisa({visaType:v.visa_type||'',visaNumber:v.visa_number||'',issueDate:v.issue_date||'',startDate:v.start_date||'',endDate:v.end_date||'',notes:v.notes||''});setError('')}}>Редактировать</button> <button onClick={()=>deleteVisa(v.id)}>Удалить</button></span></div>):<p className="muted">Нет записей</p>}</div>}
      {detailTab==='Регистрация'&&<div className="panel pad"><h2>Регистрации</h2>{selected.registrations.length?selected.registrations.map((r:any)=><div className="record" key={r.id}><span>{r.registration_type} {r.registration_number?'№'+r.registration_number:''} · до {r.end_date}</span><span><button onClick={()=>editRegistration(r)}>Редактировать</button> <button onClick={()=>deleteRegistration(r.id)}>Удалить</button></span></div>):<p className="muted">Нет записей</p>}</div>}
      {detailTab==='История'&&<div className="panel pad"><h2>История карточки</h2>{(data.foreignerHistory||[]).map((x:any)=><div className="history" key={x.id}><b>{x.action} · {x.entity_type}</b><span>{new Date(x.created_at).toLocaleString()} · {x.first_name||'система'}</span></div>)}<p className="muted">История карточки загружается из журнала операций.</p></div>}
      {detailTab==='Основная информация'&&<div className="panel pad"><h2>Прикрепленные файлы</h2><form onSubmit={saveFile} className="stack"><input type="file" onChange={readAttachment} required/><input placeholder="Имя файла" value={file.fileName} onChange={e=>setFile({...file,fileName:e.target.value})}/><input placeholder="Ссылка на файл" value={file.fileUrl} onChange={e=>setFile({...file,fileUrl:e.target.value})}/><button className="primary" disabled={savingFile}>{savingFile?"Сохранение...":"Прикрепить файл"}</button></form>{selected.files?.length?selected.files.map((x:any)=><div className="record" key={x.id}><span>{x.file_name} {x.file_size?"· "+Math.round(Number(x.file_size)/1024)+" КБ":""}</span><div><a href={x.file_url} target="_blank" rel="noreferrer">Открыть</a><button type="button" onClick={()=>deleteFile(x.id)}>Удалить</button></div></div>):<p className="muted">Файлов пока нет</p>}</div>
      {detailTab==='Основная информация'&&<div className="grid2">
  ...
  function Section(){
    ...
    if(active==='Настройки')return <div className="panel pad"><h1>Настройки</h1><h3>Профиль</h3><p><b>Пользователь:</b> {currentUser?.first_name||''} {currentUser?.last_name||''}</p><p><b>Email:</b> {currentUser?.email||'—'}</p><p><b>Роль:</b> {currentUser?.role||'—'}</p><h3>Организация</h3><p>Организация определяется текущей учетной записью и используется для изоляции данных.</p><h3>Безопасность</h3><p className="muted">Доступ к API защищён JWT и серверной проверкой роли. Изменение прав выполняется администратором.</p></div>
    ...
    if(active==='Е-паслуга')return <><h1>Е-паслуга</h1><p className="muted">Подготовка заявки на официальную государственную услугу.</p>{error&&<div className="error">{error}</div>}<div className="panel pad"><h2>Создать заявку</h2><div className="form-grid"><label>Иностранец<select value={applicationForeignerId} onChange={e=>setApplicationForeignerId(e.target.value)}><option value="">Выберите иностранца</option>{foreigners.filter((x:any)=>x.status!=='ARCHIVED').map((x:any)=><option key={x.id} value={x.id}>{x.last_name} {x.first_name} · {x.citizenship}</option>)}</select></label><label>Услуга<input value={serviceType} onChange={e=>setServiceType(e.target.value)} placeholder="Например: Регистрация иностранного гражданина"/></label></div><button className="primary" disabled={!applicationForeignerId||creatingApplication} onClick={()=>createApplication(applicationForeignerId)}>Создать заявку</button><p className="muted">Заявка получает статус подготовки к официальной отправке. Фактическая отправка выполняется через официальный государственный канал.</p></div><div className="panel"><h2 className="pad">Заявки</h2>{(data.data||[]).length?(data.data||[]).map((x:any)=><div className="history" key={x.id}><b>{x.service_type}</b><span>{x.last_name||''} {x.first_name||''} · {x.status}</span></div>):<div className="pad">Заявок пока нет.</div>}</div></>
