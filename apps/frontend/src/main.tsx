...
function App(){
  ...
  function Section(){
    ...
    if(active==='Иностранцы')return Foreigners()
      if(active==='Настройки')return <div className="panel pad"><h1>Настройки</h1><h3>Профиль</h3><p><b>Пользователь:</b> {currentUser?.first_name||''} {currentUser?.last_name||''}</p><p><b>Email:</b> {currentUser?.email||'—'}</p><p><b>Роль:</b> {currentUser?.role||'—'}</p></div>
      if(active==='Контроль сроков')return <div><h1>Контроль сроков</h1><p className="muted">Сроки регистрации, виз, документов и страхования.</p><div className="panel"><table><thead><tr><th>ФИО</th><th>Тип</th><th>Запись</th><th>Срок</th><th>Статус</th></tr></thead><tbody>{(data.data||[]).map((x:any)=><tr key={x.item_type+x.item_name+x.end_date}><td>{x.last_name} {x.first_name}</td><td>{x.item_type}</td><td>{x.item_name}</td><td>{x.end_date}</td><td>{x.deadline_status}</td></tr>)}</tbody></table></div></div>
    ...
    if(active==='История')return <><h1>История действий</h1><p className="muted">Журнал операций пользователей.</p><div className="panel">{(data.data||[]).map((x:any)=><div className="history" key={x.id}><b>{x.action} · {x.entity_type}</b><span>{new Date(x.created_at).toLocaleString()} · {x.first_name||'система'}</span></div>)}</div><div className="page-title-row"><span className="muted">Всего: {data.total||0}</span><div><button type="button" disabled={sectionPage<=1} onClick={()=>setSectionPage(p=>Math.max(1,p-1))}>← Назад</button> <span>Страница {sectionPage}</span> <button type="button" disabled={sectionPage*sectionPageSize>=Number(data.total||0)} onClick={()=>setSectionPage(p=>p+1)}>Вперёд →</button></div></div></>
      if(applicationLogs&&applicationLogsId)return <div className="panel pad"><h2>Журнал обмена</h2><button type="button" onClick={()=>{setApplicationLogs(null);setApplicationLogsId(null)}}>Закрыть</button></div>
      if(active==='Е-паслуга')return <div><h1>Е-паслуга</h1><p className="muted">Подготовка заявки на официальную государственную услугу.</p><div className="panel pad"><h2>Создать заявку</h2><p>Заявки подготавливаются для официального государственного канала.</p>{canWrite&&<button className="primary" disabled={!applicationForeignerId||creatingApplication} onClick={()=>createApplication(applicationForeignerId)}>Создать заявку</button>}</div></div>
      if(active==='Отчёты')return <div><h1>Отчёты</h1><p className="muted">Сводка по организации</p><div className="panel pad"><h2>Экспорт</h2><button type="button" onClick={exportExcel}>Экспорт Excel</button> <button type="button" onClick={exportPdf}>Экспорт PDF</button></div></div>
